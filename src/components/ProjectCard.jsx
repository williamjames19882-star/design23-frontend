...
export default function ProjectCard({ title, category, image, onClick }) {
  return (
    <motion.button
      type="button"
      onClick={onClick}
      className="group relative block w-full text-left rounded-2xl overflow-hidden bg-warm-sand/50 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-burgundy focus-visible:ring-offset-2"
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6 text-warm-white transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
        <span className="text-burgundy-light text-sm font-medium uppercase tracking-wider">{category}</span>
